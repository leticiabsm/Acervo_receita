...
<body>
    <div class="container">
        ...
        <form action="{{ route('medidas.update', $medida->idMedida) }}" method="POST">
            @csrf
            @method('PUT')

            <div>
                <label for="tipo">Tipo (ml, kg, xícara, etc.):</label>
                <input type="text" id="tipo" name="tipo" value="{{ old('tipo', $medida->tipo) }}" required>
                @error('tipo')
            ...
            <div>
                ...
                <input type="text" id="item" name="item" value="{{ old('item', $medida->item) }}" required>
                @error('item')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" id="descricao" name="descricao" value="{{ old('descricao', $medida->descricao) }}" required>
                @error('descricao')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
